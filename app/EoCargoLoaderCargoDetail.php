...
class EoCargoLoaderCargoDetail extends Model
{
    protected $table = 'eo_cargo_loader_cargo_details';
    ...
    public static function saveDetail($id, $data)
    {
        $i = -1;
        $a = 0;
        DB::table('eo_cargo_loader_cargo_details')->where('cargo_loader_id', '=', $id)->delete();
        if (isset($data['details_line'])) {

            while ($a < count($data['details_line'])) {
                $i++;
                if (isset($data['details_line'][$i])) {
                    $obj = new EoCargoLoaderCargoDetail();

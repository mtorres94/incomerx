...
class IoQuoteOriginCharge extends Model
{
    protected $table = "io_quote_origin_charges";
...
public $timestamps= false;
    public static function saveDetail($id, $data) {
        $i=-1; $a=0;
        DB::table('io_quote_origin_charges')->where('quote_id', '=', $id)->delete();
        if (isset($data['charge_id'])){

            while($a < count($data['charge_id'])){
                $i++;
                if (isset($data['charge_id'][$i])){
                    $obj = new IoQuoteOriginCharge();

...
class EoBillOfLadingHazardous extends Model
{
    protected $table = "eo_bill_of_lading_hazardous_details";
    ...
    public static function saveDetail($id, $data) {
        ...
        if (isset($data['hzd_line'])){
            $details= DB::table('eo_bill_of_lading_hazardous_details')->where('bill_of_lading_id', '=', $id)->delete();
            while($a < count($data['hzd_line'])){
                $i++;
                if (isset($data['hzd_line'][$i])){
                    $obj = new EoBillOfLadingHazardous();

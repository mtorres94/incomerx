...
class EoBillOfLadingCargoDetail extends Model
{
    protected $table = "eo_bill_of_lading_cargo_details";
    ...
    public static function saveDetail($id, $data) {
        ...
        if (isset($data['details_line'])){
            $details= DB::table('eo_bill_of_lading_cargo_details')->where('bill_of_lading_id', '=', $id)->delete();
            while($a < count($data['details_line'])){
                $i++;
                if (isset($data['details_line'][$i])){
                    $obj = new EoBillOfLadingCargoDetail();

...
class BillOfLadingCharge extends Model
{
    protected $table = "exp_bill_of_lading_charge";
    ...
    public static function saveDetail($id, $data) {
        ...
        if (isset($data['charge_id'])){
            $details= DB::table('exp_bill_of_lading_charge')->where('bill_of_lading_id', '=', $id)->delete();
            while($a < count($data['charge_id'])){
                $i++;
                if (isset($data['charge_id'][$i])){
                    $obj = new BillOfLadingCharge();

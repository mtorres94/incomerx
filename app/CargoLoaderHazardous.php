...
class CargoLoaderHazardous extends Model
{
    protected $table = "exp_cargo_loader_hazardous";
    ...
    public static function saveDetail($id, $data) {
        ...
        if (isset($data['hzd_line']) ){
            $details= DB::table('exp_cargo_loader_hazardous')->where('cargo_loader_id', '=', $id)->delete();
            while($a < count($data['hzd_line'])){
                $i++;
                if (isset($data['hzd_line'][$i])){
                    $obj = new CargoLoaderHazardous();

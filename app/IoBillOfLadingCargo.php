...
class IoBillOfLadingCargo extends Model
{
    protected $table = "io_bill_of_lading_cargo";

    protected $fillable = [
        'id', 'created_at', 'updated_at', 'line', 'bill_of_lading_id', 'container', 'cargo_type_id', 'commodity_id', 'weight_unit', 'grossw', 'cubic', 'description', 'marks', 'pieces', 'comments', 'charge_weight', 'rate', 'amount'
    ];

    public static function saveDetail($id, $data) {
        $i=0; $a=0;
        DB::table('io_bill_of_lading_cargo')->where('bill_of_lading_id', '=', $id)->delete();
        if (isset($data['cargo_id']) ){
            while($a < count($data['cargo_id'])){
                if (isset($data['cargo_id'][$i])){
                    $obj = new IoBillOfLadingCargo();

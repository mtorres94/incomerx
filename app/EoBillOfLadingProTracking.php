...
class EoBillOfLadingProTracking extends Model
{
    protected $table = "eo_bill_of_lading_pro_tracking";
    ...
    public static function saveDetail($id, $data) {
        ...
        if (isset($data['pro_line'])){
            $details= DB::table('eo_bill_of_lading_pro_tracking')->where('bill_of_lading_id', '=', $id)->delete();
            while($a < count($data['pro_line'])){
                $i++;
                if (isset($data['pro_line'][$i])){
                    $obj = new EoBillOfLadingProTracking();

...
{
    protected $table = 'whr_receipts_entries_charges_details';
    
    public $timestamps = false;

    protected $fillable = [
        'receipt_entry_id', 'line', 'billing_id', 'billing_description', 'bill_type', 'bill_party', 'billing_notes', 'billing_quantity',
    ...
    public static function createDetail($id, $data) {
        $a = 0; $i = 0;
        DB::table('whr_receipts_entries_charges_details')->where('receipt_entry_id', '=', $id)->delete();
        if (isset($data['charge_id'])){
            while ($a < count($data['charge_id']))
            {
                if (isset($data['charge_id'][$i]))
                {
                    ...
                    $obj->vendor_id = $data['billing_vendor_id'][$i];

...
<div class="form-horizontal">
    <div class="btn-group btn-group-sm pull-right" role="group" style="padding-bottom: 10px;">
        <button type="button" id="btn_cargo_details" class="btn btn-default" data-toggle="modal" data-target="#Charge_Details" onclick="cleanModalFields('Charge_Details')">
            <span class="glyphicon glyphicon-file" aria-hidden="true"></span>
        </button>
        <button type="button" class="btn btn-danger"  onclick="clearTable('charge_details'), values_charges()">
    ...
    <table class="table table-bordered table-condensed" id="charge_details">
        ...
        <tr>
            ...
            <th width="10%" data-override="Carrier">Carrier Name</th>
            ...
            <th width="10%" data-override="Amount">Amount</th>
            <th width="10%" data-override="Cost">Cost</th>
            <th width="10%"/>
        </tr>
        </thead>
        <tbody>
        @if(isset($quotes))
            @foreach($quotes->charge as $detail)
                <tr id="{{ $detail->line }}">
                    ...
                    {!! Form::bsRowTd($detail->line, 'billing_vendor_name', ($detail->vendor_id >0 ? $detail->vendor->name : ""), false) !!}
                    {!! Form::bsRowTd($detail->line, 'billing_unit_name', ($detail->unit_id >0 ? $detail->unit->code : ""), false) !!}
                    {!! Form::bsRowTd($detail->line, 'billing_bill_type', $detail->bill_type, true) !!}
                    {!! Form::bsRowTd($detail->line, 'billing_bill_party', $detail->bill_party, true) !!}
                    ...
                    {!! Form::bsRowTd($detail->line, 'billing_customer_name', ($detail->billing_customer_id >0 ? $detail->billing_customer->name : ""), true) !!}
